...
#pragma message "Adding current meaurement diagnostics"
int mcount = 0;
#endif
// DRL: End
...
int MotorDriver::getCurrentRaw(bool fromISR) {
  ...
  // DRL: Begin
#if 0
#pragma message "Printing current value for receiver"
  //if (fromISR == false) DIAG(F("%c: %d"), trackLetter, current);
  if (current) DIAG(F("%b: %c: %d"), fromISR, trackLetter, current);
#endif
  // DRL: End
...
void MotorDriver::checkPowerOverload(bool useProgLimit, byte trackno) {
  ...
  if (cCmeasure) {
     ...
     if (!mcount || trackno) {
        unsigned int mAmeasure=raw2mA(cCmeasure);
	      DIAG(F("TRACK: %c MEASURE: %dmA (%d raw)"), trackno + 'A', mAmeasure, cCmeasure);
     }
  }
  #endif
  // DRL: end
